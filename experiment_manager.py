...
class ExperimentManager:
    def __init__(self, config_fpath='config.yaml'):
        with open(config_fpath, 'r') as f:
            self.config = yaml.safe_load(f)                         # Read config.yaml file as config
        self.exps_purpose = self.config["EXPERIMENTS_PURPOSE"]      # Purpose of Experiment (Also used as wandb group)
        self.exp_configs = self.config["EXPERIMENT_CONFIGS"]        # When multiple experiments, each of experiment config as a list
        self.cache_size = self.config["CACHE_SIZE"]                 # How many hdf5 files will you send to RAM memory
        self._validate_experiments()

        # Setup results directory
        self.exps_base_dir = os.path.join(self.config["RESULTS_DIR"], f"{self.exps_purpose}")
...
# %% Setting Experiments
    def _validate_experiments(self):
        ...
        assert (len(unique_lengths) == 1), f"All keys must have same length. Found: {lengths}"
        self.num_experiments = list(lengths.values())[0]

    def get_experiment_config(self, exp_idx):
    ...
    def create_experiment_dir(self, exp_config, exp_idx):
        """Create directory structure for experiment"""
        exp_name = f'exp{exp_idx}_{exp_config["DESCRIPTION"]}'
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")

        exp_dir = os.path.join(self.exps_base_dir, f'{timestamp}_{exp_name}_gpu{exp_config["GPU_NUM"]}')
    ...
    def save_experiment_config(self, exp_config, exp_dirs):
...
# %% Initialisation of WandB
    def init_wandb(self, exp_config, exp_idx, exp_dirs):
...
# %% Running Experiments
    def run_experiment(self, exp_idx):
        ...
        develop_dataloaders, global_class_mapping = setup_develop_dataloaders(exp_config)  # ✨ MODIFIED
        ...
        print(f"\n{'=' * 70}")
        print(f" - NUM_CLASSES auto-set to {exp_config['NUM_CLASSES']} (DATASET_MODE: {exp_config.get('DATASET_MODE', 'Total')})")
        print(f"{'=' * 70}")
        # Initialise wandb
        ...
        wandb.log({
            'total_params': total_params, 'trainable_params': trainable_params,                                     # parameters
            'training_time_sec': train_elapsed, 'avg_epoch_time_sec': train_results['avg_epoch_time'],              # training time / avg epoch time
            'best_val_loss': train_results['best_val_loss'],                                                        # best validation loss
            'peak_gpu_memory_gb': torch.cuda.max_memory_allocated() / 1e9 if torch.cuda.is_available() else 0,      # gpu allocated memory
            'eval_loss': eval_results['eval_loss'],                                                                 # evaluation loss
            'eval_accuracy': eval_results['eval_accuracy'],                                                         # eval combined accuracy
            'azimuth_accuracy': eval_results['overall_results']['azimuth_accuracy'],                                # eval azimuth accuracy
            'elevation_accuracy': eval_results['overall_results']['elevation_accuracy'],                            # eval elevation accuracy
            'spherical_error': eval_results['overall_results']['spherical_error'],                                  # eval spherical error
            'azimuth_error': eval_results['overall_results']['azimuth_error'],                                      # eval azimuth error
            'elevation_error': eval_results['overall_results']['elevation_error']                                   # eval elevation error
        })

        # Log SNR-wise eval results as line plots
        metric_names = [
            'spherical_error', 'azimuth_error', 'elevation_error',
