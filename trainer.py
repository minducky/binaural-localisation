...
class Trainer:
    def __init__(self, model, config, dataloaders, exp_dirs, plotter):
        self.model = model
        self.config = config
    ...
    def train_epoch(self, enable_profiler=False):
        self.model.train()
        epoch_loss = 0

        pbar = tqdm(self.dataloaders["train"], desc="Training", leave=True, ncols=100, colour="blue")
        ...
        if enable_profiler:
            ...
            print(prof.key_averages(group_by_stack_n=5).table(sort_by="cuda_time_total", row_limit=50))
    ...
    def validate_epoch(self):
        ...
        with torch.no_grad():
            pbar = tqdm(self.dataloaders["val"], desc="Validation", leave=True, ncols=100, colour="magenta")
    ...
    def evaluate(self, eval_dataloader):
        ...
        with torch.no_grad():
            ...
            for x, y, azim, elev, snr in pbar:  # now includes elev and snr (only when evaluation)
                x, y = x.to(self.device), y.to(self.device)
                ...
                for pred_cls, true_cls in zip(preds, labels):
        ...
        print(f"  Unique azimuths: {len(unique_eval_azims)} (out of {len(base_dataset.azim_values)} in training)")
        print(f"  Azimuth range: {unique_eval_azims.min():.1f}° to {unique_eval_azims.max():.1f}°")
        print(f"  Unique elevations: {len(unique_eval_elevs)} (out of {len(base_dataset.elev_values)} in training)")
        print(f"  Elevation range: {unique_eval_elevs.min():.1f}° to {unique_eval_elevs.max():.1f}°")
        print(f"  Unique classes: {len(unique_eval_classes)} (out of {len(class_to_angle)} in training)")
        ...
        print(f"\n{'='*70}")
        print(f"Evaluation Results by SNR")
        print(f"{'='*70}")

        for snr_val in sorted(unique_snrs):
            mask = all_snrs == snr_val
            snr_results = self._calculate_metrics(
                all_pred_azims[mask], all_pred_elevs[mask],
                all_true_azims[mask], all_true_elevs[mask],
        ...
        overall_results = self._calculate_metrics(
            all_pred_azims, all_pred_elevs,
            all_true_azims, all_true_elevs,
            all_preds, all_labels
        )

        print(f"\n{'='*70}")
        print(f"Overall Results (All SNRs)")
        print(f"{'='*70}")
        ...
        eval_metadata = {
            ...
            'azim_range': (float(unique_eval_azims.min()), float(unique_eval_azims.max())),
            'elev_range': (float(unique_eval_elevs.min()), float(unique_eval_elevs.max())),
        ...
        conf_matrices = self._build_confusion_matrices(
            all_true_azims, all_pred_azims,
            all_true_elevs, all_pred_elevs,
            all_labels, all_preds, class_to_angle,
        )
        ...
        self._save_evaluation_results(
            results_by_snr, overall_results, eval_loss, len(eval_dataloader["eval"]),
            eval_metadata, conf_matrices,
        )
    ...
    def _calculate_metrics(self, pred_azims, pred_elevs, true_azims, true_elevs, pred_classes, true_classes):
    ...
    def _spherical_distance(self, azim1, elev1, azim2, elev2):
        ...
        a = np.sin(dlat/2)**2 + np.cos(elev1_rad) * np.cos(elev2_rad) * np.sin(dlon/2)**2
        c = 2 * np.arcsin(np.sqrt(a))

        return np.rad2deg(c)

    def _build_confusion_matrices(self, true_azims, pred_azims, true_elevs, pred_elevs,
                                   true_classes, pred_classes, class_to_angle):
        def _to_signed(a):
        ...
        class_tick_labels = [f"e{class_to_angle[c][1]:.0f}/a{_to_signed(class_to_angle[c][0]):.0f}"
                             for c in sorted_classes]

        return {
            'cm_azim':  cm_azim,  'azim_labels':  [str(int(a)) for a in azim_labels],
            'cm_elev':  cm_elev,  'elev_labels':  elev_labels,
            'cm_class': cm_class, 'class_labels': class_tick_labels,
        }

    def _save_evaluation_results(self, results_by_snr, overall_results, eval_loss, num_batches,
                                  eval_metadata, conf_matrices):
    ...
    def _get_bm_params(self):
        from models.ducky import BM
        for module in self.model.modules():
            if isinstance(module, BM) and module.learnable_coefficients:
                return module.get_learned_params()
        return None

    def _save_bm_history(self):
        if not self.bm_fc_history:
            return
        history_dir = self.exp_dirs['history']
        # shape: (num_epochs, num_filters)
        np.save(os.path.join(history_dir, 'bm_fc_history.npy'), np.stack(self.bm_fc_history))
        np.save(os.path.join(history_dir, 'bm_ourbeta_history.npy'), np.stack(self.bm_ourbeta_history))
