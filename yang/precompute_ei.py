...
from dataset import BinauralDataset  # noqa: E402
from models.yang import YangBM, YangEI, YangIHC, YangOM  # noqa: E402
...
class CochlearPipeline(torch.nn.Module):
    ...
    def __init__(self):
        super().__init__()
        filter_coeff_dir = os.path.join(SCRIPT_DIR, "filter_coeff_dir")
        ...
        self.basilar = YangBM(filter_coeff_dir=filter_coeff_dir, sr=sr, trainable=False)
        self.ihc = YangIHC(
...
def precompute_split(pipeline, dataset, device, batch_size, num_workers, output_path):
    ...
    offset = 0
    pbar = tqdm(
        dataloader, desc=f"Computing EI -> {os.path.basename(output_path)}", ncols=120
    )
    ...
    with h5py.File(output_path, "w") as f:
        f.create_dataset(
            "signal_ei", data=all_ei, compression="gzip", compression_opts=4
        )
...
def main():
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="/scratch/dm1u25/dataset/Saddler_EI",
    ...
    train_dataset = BinauralDataset(
        config, "train", cache_size=config["CACHE_SIZE"]["TRAIN"]
    )
    ...
    save_class_mapping(
        global_class_mapping, os.path.join(args.output_dir, "class_mapping.hdf5")
    )

    # --- Valid split ---
    print("\n=== Valid split ===")
    valid_dataset = BinauralDataset(
        config,
        "valid",
        cache_size=config["CACHE_SIZE"]["VAL"],
