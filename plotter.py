...
class ResultPlotter:
    ...
    def plot_evaluation_results(self, results_by_snr, overall_results, save_dir, eval_metadata=None):
        ...
        for snr_key in sorted(results_by_snr.keys(), key=lambda k: float(k.replace("SNR_", ""))):
        ...
        # ✨ NEW: Add metadata to title if available
        title_suffix = ""
        if eval_metadata:
            title_suffix = f"<br><sub>{eval_metadata['num_unique_azims']} azim × {eval_metadata['num_unique_elevs']} elev = {eval_metadata['num_unique_classes']} classes</sub>"
        ...
        fig2.update_layout(
            ...
            xaxis=dict(title="SNR (dB)", tickfont=dict(size=12), showgrid=True, gridcolor='lightgray', zeroline=False),
            yaxis=dict(title="Accuracy", tickfont=dict(size=12), tickformat=".0%", showgrid=True,
                       gridcolor='lightgray'),
            width=1000,
            height=600,
            paper_bgcolor="white",
    ...
    def plot_confusion_matrices(self, conf_matrices, save_dir):
        ...
        specs = [
            ...
            ('cm_class', 'class_labels', 'Class Confusion Matrix (sorted by elev, azim)', 'confusion_class'),
        ]

        for cm_key, label_key, title, fname in specs:
            cm = conf_matrices[cm_key].astype(float)
            labels = [str(l) for l in conf_matrices[label_key]]
