"""Conv2D baseline models used as compute/parameter-count reference points."""
...
class Dummy5M(nn.Module):
    ...
    def __init__(self, config: dict):
        ...
        self.conv = nn.Sequential(
            nn.Conv2d(
                1, 128, kernel_size=(64, 2), stride=(16, 1)
            ),  # (B, 128,  2753, 1)
            nn.ReLU(),
            nn.Conv2d(
                128, 256, kernel_size=(16, 1), stride=(8, 1)
            ),  # (B, 256,  343,  1)
            nn.ReLU(),
            nn.Conv2d(
                256, 512, kernel_size=(8, 1), stride=(4, 1)
            ),  # (B, 512,  84,   1)
            nn.ReLU(),
            nn.Conv2d(
                512, 1024, kernel_size=(4, 1), stride=(2, 1)
            ),  # (B, 1024, 41,   1)
            nn.ReLU(),
...
class Dummy10M(nn.Module):
    ...
    def __init__(self, config: dict):
        ...
        self.conv = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Conv2d(
                1024, 1024, kernel_size=(4, 1), stride=(2, 1)
            ),  # (B, 1024, 19,   1)
            nn.ReLU(),
