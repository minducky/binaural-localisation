"""Datasets and dataloader setup for binaural-localisation train/val/eval splits."""
...
class BinauralDataset(Dataset):
    """Raw-waveform binaural dataset backed by per-file HDF5 stimulus shards."""
    ...
    def __init__(
        ...
        Args:
            ...
            global_class_mapping: Optional class mapping reused from the
                training set; if omitted, a mapping is loaded from disk or
                built from scratch.
        """
        self.data_dir = os.path.join(config["DATASET_DIR"], mode)
        self.h5_fnames = sorted(
        ...
        if global_class_mapping is not None:
            ...
            print(
                f"     - Loaded {len(self.class_to_angle)} classes from global mapping"
            )
        else:
    ...
    def _load_or_build_angle_mappings(self) -> None:
        ...
        if os.path.exists(mapping_fpath):
            ...
            self.angle_to_class = {}
            self.class_to_angle = {}
            for a, e, c in zip(mapping_azim, mapping_elev, mapping_class, strict=True):
    ...
    def build_eval_prior(self, n_classes: int) -> np.ndarray:
    ...
    def __getitem__(
        ...
        """Returns ``(signal, class_idx, azim, elev, snr)`` for sample ``idx``."""
...
class EIDataset(Dataset):
    """Precomputed excitation-inhibition (EI) feature dataset for YangMLP."""
    ...
    def __init__(
        ...
        if global_class_mapping is not None:
            ...
            raise ValueError(
                "Either global_class_mapping or class_mapping_path must be provided"
            )
    ...
    def __getitem__(
        ...
        """Returns ``(signal_ei, class_idx, azim, elev, snr)`` for sample ``idx``."""
        return (
...
class FileGroupedSampler(Sampler):
    """Samples indices grouped by source HDF5 file, to reduce h5 I/O."""

    def __init__(self, dataset: Dataset | Subset, shuffle: bool = True):
        """Groups sample indices by their source HDF5 file.

        Args:
            dataset: A ``BinauralDataset``/``EIDataset`` (or a ``Subset`` of
                one) exposing an ``index_map`` of ``(h5_file, sample_idx)``.
            shuffle: If True, shuffle both the file-group order and the
                samples within each group on every iteration.
        """
        # Subset wrapping
        base = (
...
def setup_develop_dataloaders(config: dict) -> tuple[dict, dict]:
    """Builds train/val dataloaders and the (auto-built) global class mapping.

    Args:
        config: Experiment config dict.

    Returns:
        A tuple ``(dataloaders, global_class_mapping)``: a dict with
        ``'train'``/``'val'`` ``DataLoader``s, and the class mapping built
        from the training set (to be reused for eval).
    """
    ...
    if use_ei:
        ...
        print("\n [[[ Validation ]]] ")
        val_dataset = EIDataset(
            os.path.join(ei_dir, "valid_ei.hdf5"),
            config=config,
            mode="valid",
        ...
        val_dataset = BinauralDataset(
            config, "valid", config["CACHE_SIZE"]["VAL"], global_class_mapping
        )
    ...
    if config["DEBUG"]:
        train_dataset = Subset(
            train_dataset, range(config["DEBUG_CONFIG"]["TRAIN_DATASET_SIZE"])
        )
        val_dataset = Subset(
            val_dataset, range(config["DEBUG_CONFIG"]["VAL_DATASET_SIZE"])
        )
        train_size = config["DEBUG_CONFIG"]["TRAIN_DATASET_SIZE"]
        val_size = config["DEBUG_CONFIG"]["VAL_DATASET_SIZE"]
        print(f"\n{'-' * 70}")
        print(
            f"Debug mode : \n"
            f"- ************* train dataset into {train_size} samples\n"
            f"- ************* val dataset into {val_size} samples\n"
        )
        print(f"{'-' * 70}")
...
def setup_eval_dataloaders(config: dict, global_class_mapping: dict) -> dict:
    """Builds the eval dataloader, reusing the class mapping from training.

    Args:
        config: Experiment config dict.
        global_class_mapping: Class mapping built by
            ``setup_develop_dataloaders`` from the training set.

    Returns:
        A dict with a single ``'eval'`` ``DataLoader``.
    """
    use_ei = config["MODEL"] == "YangMLP"
    ...
    if use_ei:
        ...
        eval_dataset = EIDataset(
            os.path.join(ei_dir, "eval_ei.hdf5"),
            config=config,
            mode="eval",
            global_class_mapping=global_class_mapping,
        )
        ...
        eval_dataset = BinauralDataset(
            config,
            "evaluation/v01_eval_mit_bldg46room1004_tenoise",
            config["CACHE_SIZE"]["EVAL"],
            global_class_mapping,
        )
    ...
    if config["DEBUG"]:
        eval_dataset = Subset(
            eval_dataset, range(config["DEBUG_CONFIG"]["EVAL_DATASET_SIZE"])
        )
        eval_size = config["DEBUG_CONFIG"]["EVAL_DATASET_SIZE"]
        print(f"Debug mode : \n- eval dataset into {eval_size} samples\n")
    return {
        "eval": DataLoader(
